...
public class ReservationRequestHandler implements Runnable {
    ...
    public synchronized void airlinePassengers(String airlineChoice) {
        ...
        if (airlineChoice.equals(Alaska.name)) {
            ...
            ReservationServer.getDeltaInfo();
            ReservationServer.getDeltaPassenger();
            JLabel title = new JLabel("Delta Airline." + Delta.numSeat);
            JTextArea result = new JTextArea();
            String text = "";
            titlePanel.add(title);
            for (int i = 0; i < Delta.passenger.size(); i++) {
                text = text + Delta.passenger.get(i) + "\n";
            }
            result.setText(text);
            display.add(result);
        } else if (airlineChoice.equals(Southwest.name)) {
            ReservationServer.getSouthwestInfo();
            ReservationServer.getSouthwestPassenger();
            JLabel title = new JLabel("Southwest Airline." + Southwest.numSeat);
            JTextArea result = new JTextArea();
            String text = "";
            titlePanel.add(title);
            for (int i = 0; i < Southwest.passenger.size(); i++) {
                text = text + Southwest.passenger.get(i) + "\n";
            }
            result.setText(text);
            display.add(result);
        }
